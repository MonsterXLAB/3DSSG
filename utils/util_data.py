...
def build_edge_from_selection(node_ids: set, neighbor_dict: dict,
                              max_edges_per_node: int):
    '''Generate edges [[obji, objj], ...] from node_ids in the scan.

    flow: an edge passes message from i to j is denoted as  [i,j]. 

    Args:
        node_ids(set{int}): filterred nodes.
        neighbor_dict(dict{str:list[int]}): All the neighbors relationships in the sacn.
        max_edges_per_node(int):
    '''
    ''' build trees '''
    edge_indices = list()
    for s_idx in node_ids:
        ...
        nn = set(nn).intersection(
            node_ids)  # only the nodes within node_ids are what we want
...
def build_neighbor(neighbors_dict: dict,
    ...
    Returns:
        filterred_nodes(set): neighbors nodes in the range of n_times levels.
    '''
    selected_nodes = list(instance2labelName.keys())
    index = np.random.choice(np.unique(selected_nodes), n_seed).tolist()
    index = list(
        set(index)
    )  # TODO dict keys can't be the same, so the indexs are unique already.
    # Check all the random selected objects are in the graph.
    for n_idx in selected_nodes:
        if str(n_idx) not in neighbors_dict:
            print('cannot find key', n_idx, 'in', neighbors_dict.keys())
            assert str(n_idx) in neighbors_dict.keys()
    ''' loop over n times'''
    filtered_nodes = set()
    n_seletected_nodes = dict(
    )  # this save the neighbor with level n. level n+1 is the neighbors from level n.
    ...
    for n in range(n_times):
        ...
        for node_idx in n_seletected_nodes[n]:
            found = set(neighbors_dict[str(node_idx)])
            found = found.intersection(
                selected_nodes)  # only choose the node within our selections
...
def data_preparation(
        ...
        # use_rgb, use_normal,
        for_train=False,
        instance2labelName=None,
        classNames=None,
        rel_json=None,
        relationships=None,
        multi_rel_outputs=None,
        use_all=False,
        padding=0.2,
        num_max_rel=-1,
        shuffle_objs=True,
        nns: dict = None,
        sample_in_runtime: bool = False,
        num_nn=1,
        num_seed=1):
    ...
    if sample_in_runtime:
        if not use_all:
            filtered_nodes = build_neighbor(nns, instance2labelName, num_nn,
                                            num_seed)
        else:
            selected_nodes = list(instance2labelName.keys())
            filtered_nodes = selected_nodes  # use all nodes
        edge_indices = build_edge_from_selection(filtered_nodes,
                                                 nns,
    ...
    for instance_id in list(np.unique(instances)):
        ...
        if for_train:
            ...
            if (class_id >= 0) and (instance_id >
                                    0):  # there is no instance 0?
                cat.append(class_id)
        else:
            class_id = 0
    ...
    num_objects = len(instances_id) if selected_instances is None else len(
        selected_instances)

    masks = np.array(list(map(lambda l: instance2mask[l], instances)),
                     dtype=np.int32)
    ...
    for e in range(len(edge_indices)):
        ...
        choice = np.random.choice(len(pointset),
                                  num_points_union,
                                  replace=True)
