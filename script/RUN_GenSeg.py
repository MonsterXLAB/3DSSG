...
exe_path = '/3DSSG/SceneGraphFusion/bin/exe_GraphSLAM'

parser = argparse.ArgumentParser(
    description='Generate InSeg segmentation on 3RScan or ScanNet dataset.',
    formatter_class=argparse.ArgumentDefaultsHelpFormatter)
parser.add_argument('--min_pyr_level',
                    type=int,
                    default=2,
                    help='',
                    required=False)
parser.add_argument('--depth_edge_threshold',
                    type=float,
                    default=0.98,
                    help='',
                    required=False)
...
parser.add_argument('--type',
                    ...
                    help="which split of scans to use",
                    required=True)
...
parser.add_argument('--rendered',
                    type=int,
                    default=1,
                    help='use rendered depth (for ScanNet and 3RScan)',
                    required=False)
    ...
    try:
        output = subprocess.check_output(
            [
                ...
                str(args.rendered),
                '--min_pyr_level',
                str(args.min_pyr_level),  # was 2
                '--depth_edge_threshold',
                str(args.depth_edge_threshold),
                '--save',
                str(1),
                '--save_graph',
                '1',
        ...
        print('[Catched Error]',
              "command '{}' return with error (code {}): {}".format(
                  e.cmd, e.returncode,
                  e.output.decode('utf-8')))  # omit errors
    endTime = time.time()
...
if __name__ == '__main__':
    ...
    if args.dataset == '3RScan':
        ...
        train_scans, val_scans = gen_splits_scannet(define.SCANNET_SPLIT_TRAIN,
    ...
    for scan_id in sorted(scan_ids):
        ...
        if args.dataset == '3RScan':
            ...
            pth_in = os.path.join(define.SCANNET_DATA_PATH, scan_id,
                                  scan_id + '.sens')
        ...
        if args.thread > 1:
            results.append(pool.apply_async(process, (pth_in, pth_out)))
        else:
