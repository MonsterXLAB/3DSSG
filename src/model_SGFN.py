...
class SGFNModel(BaseModel):
    def __init__(self,
        ...
        if self.mconfig.USE_GCN:
            if mconfig.GCN_TYPE == "TRIP":
                ...
                models['gcn'] = GraphEdgeAttenNetworkLayers(
        ...
        if self.config.LR_SCHEDULE == 'BatchSize':
            ...
            self.scheduler = optimizer.BatchMultiplicativeLR(
                self.optimizer, _scheduler)
    ...
    def forward(self,
        ...
        ''' Create edge feature '''
        with torch.no_grad():
            edge_feature = op_utils.Gen_edge_descriptor(flow=self.flow)(
                descriptor, edges)
        ...
        if self.mconfig.USE_GCN:
            if self.mconfig.GCN_TYPE == 'TRIP':
                gcn_obj_feature, gcn_rel_feature = self.gcn(
                    obj_feature, rel_feature, edges)
            elif self.mconfig.GCN_TYPE == 'EAN':
                gcn_obj_feature, gcn_rel_feature, probs = self.gcn(
                    obj_feature, rel_feature, edges)
        else:
    ...
    def process(self,
        ...
        obj_pred, rel_pred, _, _, _, _, probs = self(obj_points,
                                                     edges,
                                                     descriptor,
                                                     return_meta_data=True,
                                                     imgs=imgs,
                                                     covis_graph=covis_graph)

        if self.mconfig.multi_rel_outputs:
            if self.mconfig.WEIGHT_EDGE == 'BG':
                if self.mconfig.w_bg != 0:
                    weight = self.mconfig.w_bg * (1 - gt_rel_cls) + (
                        1 - self.mconfig.w_bg) * gt_rel_cls
                else:
                    weight = None
                ...
                weight = torch.abs(1.0 / (torch.log(batch_mean + 1) +
                                          1))  # +1 to prevent 1 /log(1) = inf
                if ignore_none_rel:
                    weight[0] = 0
                ...
                ) if not ignore_none_rel else 0  # * 1e-3
                weight = weight[1:]

            else:
                raise NotImplementedError("unknown weight_edge type")

            loss_rel = F.binary_cross_entropy(rel_pred,
                                              gt_rel_cls,
                                              weight=weight)
        else:
            if self.mconfig.WEIGHT_EDGE == 'DYNAMIC':
                one_hot_gt_rel = torch.nn.functional.one_hot(
                    gt_rel_cls, num_classes=self.num_rel)
                batch_mean = torch.sum(one_hot_gt_rel,
                                       dim=(0),
                                       dtype=torch.float)
                weight = torch.abs(1.0 / (torch.log(batch_mean + 1) +
                                          1))  # +1 to prevent 1 /log(1) = inf
            ...
            if 'ignore_entirely' in self.mconfig and (
                    self.mconfig.ignore_entirely and ignore_none_rel):
                loss_rel = torch.zeros(1,
        ...
        if 'USE_REL_LOSS' in self.mconfig and not self.mconfig.USE_REL_LOSS:
            loss = loss_obj
        elif 'ignore_entirely' in self.mconfig and (
                self.mconfig.ignore_entirely and ignore_none_rel):
            loss = loss_obj
        else:
    ...
    def calculate_metrics(self, preds, gts):
        ...
        logs = [("Accuracy/obj_cls", acc_obj), ("Accuracy/rel_cls", acc_rel)]
        return logs
        ...
        if self.mconfig.USE_GCN:
            ...
            if self.mconfig.GCN_TYPE == 'EAN':
                for i in range(self.gcn.num_layers):
                    params['gcn_' + str(i)] = self.gcn.gconvs[i].trace(
                        path, 'gcn_' + str(i))
            else:
                raise NotImplementedError()
...
if __name__ == '__main__':
    ...
    network = SGFNModel(config, 'SceneGraphFusionNetwork', num_obj_cls,
                        num_rel_cls)
    ...
    if not use_dataset:
        ...
        network.process(obj_points,
                        edge_indices.t().contiguous(), obj_gt, rel_gt)

    for i in range(100):
        if use_dataset:
            scan_id, instance2mask, obj_points, edge_indices, obj_gt, rel_gt = dataset.__getitem__(
                i)

        logs, obj_pred, rel_pred = network.process(
            obj_points,
            edge_indices.t().contiguous(), obj_gt, rel_gt)
        logs += network.calculate_metrics([obj_pred, rel_pred],
                                          [obj_gt, rel_gt])
