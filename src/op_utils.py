...
def gen_descriptor(pts: torch.tensor):
    ...
    # volume [n, 1]
    segment_volume = (segment_dims[0] * segment_dims[1] *
                      segment_dims[2]).unsqueeze(0)
    # length [n, 1]
...
class Gen_edge_descriptor(MessagePassing):  #TODO: move to model
    ...
    def forward(self, descriptor, edges_indices):
        ...
        coll_dict = self.__collect__(self.__user_args__, edges_indices, size,
                                     {"x": descriptor})
...
def check(x, y):
    ...
    [
        np.testing.assert_allclose(x[i].flatten(),
                                   y[i].flatten(),
                                   rtol=1e-03,
                                   atol=1e-05) for i in range(len(x))
    ]


def export(model: torch.nn.Module, inputs: list, pth: str, input_names: list,
           output_names: list, dynamic_axes: dict):
    import onnxruntime as ort
    inputs = inputs if isinstance(inputs, list) or isinstance(
        inputs, tuple) else [inputs]
