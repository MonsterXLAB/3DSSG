...
def load_mesh(path, label_file, use_rgb, use_normal):
    ...
    if label_file == 'labels.instances.align.annotated.v2.ply' or label_file == 'labels.instances.align.annotated.ply':
        if use_rgb:
            ...
            plydata = trimesh.load(os.path.join(path, label_file),
                                   process=False)
        ...
        points = np.array(plydata.vertices)
        instances = plydata.metadata['ply_raw']['vertex']['data'][
            'label'].flatten()
...
def dataset_loading_3RScan(root: str,
                           pth_selection: str,
                           split: str,
    ...
    if split == 'train_scans':
        selected_scans = selected_scans.union(
            util.read_txt_to_list(
                os.path.join(pth_selection, 'train_scans.txt')))
        with open(os.path.join(root, 'relationships_train.json'),
                  "r") as read_file:
            data1 = json.load(read_file)
    elif split == 'validation_scans':
        selected_scans = selected_scans.union(
            util.read_txt_to_list(
                os.path.join(pth_selection, 'validation_scans.txt')))
        with open(os.path.join(root, 'relationships_validation.json'),
                  "r") as read_file:
            data1 = json.load(read_file)
    elif split == 'test_scans':
        selected_scans = selected_scans.union(
            util.read_txt_to_list(os.path.join(pth_selection,
                                               'test_scans.txt')))
        with open(os.path.join(root, 'relationships_test.json'),
                  "r") as read_file:
            data1 = json.load(read_file)
    else:
    ...
    if 'neighbors' in data1:
        data['neighbors'] = data1[
            'neighbors']  #{**data1['neighbors'], **data2['neighbors'], **data3['neighbors']}
...
class SGFNDataset(data.Dataset):
    def __init__(self,
        ...
        wobjs, wrels, o_obj_cls, o_rel_cls = compute_weight_occurrences.compute(
            self.classNames, self.relationNames, data, selected_scans)
        self.w_cls_obj = torch.from_numpy(np.array(o_obj_cls)).float().to(
            self.config.DEVICE)
        self.w_cls_rel = torch.from_numpy(np.array(o_rel_cls)).float().to(
            self.config.DEVICE)
        ...
        if False:
            ...
            ''' inverse sum'''
            self.w_cls_obj = self.w_cls_obj.sum() / (self.w_cls_obj +
                                                     1) / self.w_cls_obj.sum()
            self.w_cls_rel = self.w_cls_rel.sum() / (self.w_cls_rel +
                                                     1) / self.w_cls_rel.sum()
        ...
        if self.config.VERBOSE:
            ...
            for i in range(len(self.classNames)):
                print('|{0:>3d} {1:>20s}'.format(i, self.classNames[i]),
                      end='')
            ...
            for i in range(len(self.relationNames)):
                print('|{0:>3d} {1:>20s}'.format(i, self.relationNames[i]),
                      end=' ')
        ...
        self.relationship_json, self.objs_json, self.scans, self.nns = self.read_relationship_json(
            data, selected_scans)
        if self.config.VERBOSE:
            ...
            for scan_id in self.scans:
                ...
                if scan_id_no_split not in self.cache_data:
                    self.cache_data[scan_id_no_split] = pool.apply_async(
                        load_mesh, (path, self.mconfig.label_file,
    ...
    def data_augmentation(self, points):
        # random rotate
        matrix = np.eye(3)
        matrix[0:3, 0:3] = op_utils.rotation_matrix([0, 0, 1],
                                                    np.random.uniform(
                                                        0, 2 * np.pi, 1))
        ...
        if self.use_normal:
            ...
            if self.use_rgb:
                ofset += 3
            points[:, ofset:3 + ofset] = np.dot(points[:, ofset:3 + ofset],
                                                matrix.T)
    ...
    def __getitem__(self, index):
        ...
        if self.load_cache:
            ...
            if 'scene' in scan_id:
                ...
                path = os.path.join(self.root_3rscan, scan_id_no_split)
            data = load_mesh(path, self.mconfig.label_file, self.use_rgb,
                             self.use_normal)
        points = data['points']
        ...
        if self.sample_in_runtime:
            if not self.for_eval:
                ...
                filtered_nodes = util_data.build_neighbor(
                    self.nns[scan_id_no_split], instance2labelName,
                    sample_num_nn, sample_num_seed
            ...
            edge_indices = util_data.build_edge_from_selection(
                filtered_nodes,
            ...
            if self.mconfig.drop_edge > 0 and not self.for_eval:
                percentage = np.random.uniform(low=1 - self.mconfig.drop_edge,
                                               high=1.0,
                                               size=1)[0]
                num_edge = int(float(len(edge_indices)) * percentage // 1)
                if num_edge > 0:
                    choices = np.random.choice(range(len(edge_indices)),
                                               num_edge,
        ...
        if self.sample_in_runtime:
            edge_indices = [[
                instance2mask[edge[0]] - 1, instance2mask[edge[1]] - 1
            ...
            if max_edges > 0 and len(edge_indices) > max_edges:
                ...
                indices = list(
                    np.random.choice(len(edge_indices),
                                     self.max_edges,
                                     replace=False))
        ...
        obj_points = torch.zeros(
            [len(cat), self.mconfig.num_points, self.dim_pts])
        ...
        for i in range(len(filtered_instances)):
            ...
            descriptor[i] = op_utils.gen_descriptor(
        ...
        if self.multi_rel_outputs:
            adj_matrix_onehot = np.zeros(
                [len(cat), len(cat),
                 len(self.relationNames)])
        else:
        ...
        if not self.sample_in_runtime:
            ...
            for n in range(len(cat)):
                for m in range(len(cat)):
                    if n == m: continue
                    edge_indices.append([n, m])
            if max_edges > 0 and len(
                    edge_indices) > max_edges and not self.for_eval:
                # for eval, do not drop out any edges.
                indices = list(
                    np.random.choice(len(edge_indices),
                                     max_edges,
        ...
        if self.multi_rel_outputs:
            ...
            adj_matrix_onehot = torch.from_numpy(
                np.array(adj_matrix_onehot, dtype=rel_dtype))
        else:
            rel_dtype = np.int64
            adj_matrix = torch.from_numpy(np.array(adj_matrix,
                                                   dtype=rel_dtype))

        if self.multi_rel_outputs:
            gt_rels = torch.zeros(len(edge_indices),
                                  len(self.relationNames),
                                  dtype=torch.float)
        else:
    ...
    def norm_tensor(self, points):
        ...
        furthest_distance = points.pow(2).sum(
            1).sqrt().max()  # find maximum distance for each n -> [n]
